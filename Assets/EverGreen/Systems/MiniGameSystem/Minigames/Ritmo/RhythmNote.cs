...
public class RhythmNote : MonoBehaviour
{
    ...
    {
        RectTransform rect = GetComponent<RectTransform>();
        RectTransform areaRect = transform.parent.GetComponent<RectTransform>();
        return rect.anchoredPosition.y < -areaRect.rect.height / 2f - rect.rect.height;
    }

    public float GetVerticalDistanceToHitZone()
    {
        RectTransform rect = GetComponent<RectTransform>();
        // Considera Y=0 como zona de acerto
        return Mathf.Abs(rect.anchoredPosition.y);
    }
